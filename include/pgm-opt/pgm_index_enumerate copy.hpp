...
namespace pgm_sequence {
    ...
    class pgm_enumerator{
        ...
        void load_copy(uint64_t data_size, std::vector<Correction_Value> corrections_vector) {
            ...
            if (segments.size() > 1)
                this -> next_first_value = segments[1].intercept + corrections_vector[segments[1].first]; // unused
            else
                this -> next_first_value = UINT32_MAX;
        ...
        K nextgeq_new(K posting_value) {
            if (current_value >= posting_value) {
                return current_value;
            }

            for (auto it = segments.begin() + current_segment; it < segments.end(); ++it) {
                const size_t idx = static_cast<size_t>(it - segments.begin());
                const auto first = it->first;
                const auto covered = it->covered;

                // 计算“下一段”的首值（与当前 it 绑定）
                K next_seg_first = std::numeric_limits<K>::max();
                if (auto it_next = std::next(it); it_next < segments.end()) {
                    next_seg_first = it_next->intercept + corrections_vector[it_next->first];
                }

                // 若查询值已不小于下一段首值，则本段不可能有答案，整段跳过
                if (posting_value >= next_seg_first) {
                    if (idx > current_segment) current_segment = idx;
                    current_pos = first + covered;  // 游标推进到本段末
                    continue;
                }

                // 在本段内扫描
                if (current_segment != idx) {
                    current_segment = idx;
                    current_pos = first;
                    current_correction = it->intercept;
                }

                const auto slope_significand = it->slope_significand;
                const auto slope_exponent = it->slope_exponent;

                for (Covered_Value j = current_pos - first; j < covered; ++j) {
                    current_correction += corrections_vector[first + j];
                    current_value = ((j * slope_significand) >> slope_exponent) + current_correction;
                    if (current_value >= posting_value) {
                        current_pos = first + j + 1;
                        return current_value;
                    }
                }

                // 本段耗尽，进入下一段
                current_pos = first + covered;
            }
